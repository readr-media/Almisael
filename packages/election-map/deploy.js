...
async function run() {
  ...
  try {
    ...
    const { organizations, environments } =
      await configService.getValidOptions()
    ...
    const dryRun =
      process.env.DRY_RUN === 'true' || (await validationService.promptDryRun())
    ...
    if (dryRun) {
      logger.info(
        'Running in DRY RUN mode - builds will be created but not deployed to GCS'
      )
    }
...
async function processDeployment(
  ...
  try {
    ...
    const deployResult = await gcsService.deployToGCS(
      org,
      env,
      outDir,
      deployConfig,
    ...
    logger.error(`Deployment failed for ${org} (${env})`, {
      error: error.message,
    })
